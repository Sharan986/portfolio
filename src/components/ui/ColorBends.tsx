...
void main() {
  ...
  // Aspect ratio correction (uniform scale)
...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    // Clamp to 1.0 — shader art is visually indistinguishable at retina
    // but costs 4× GPU at 2× DPR
    ...
    const loop = () => {
      if (!isVisibleRef.current) {
        // Offscreen — stop the loop entirely, will restart on intersection
        rafRef.current = null;
        return;
      }
      timer.update();
    ...
    // Visibility-gated rendering via IntersectionObserver
    const io = new IntersectionObserver(
      (entries) => {
        const isNowVisible = entries[0]?.isIntersecting ?? false;
        const wasVisible = isVisibleRef.current;
        isVisibleRef.current = isNowVisible;
        // Restart the loop when becoming visible
        if (isNowVisible && !wasVisible && rafRef.current === null) {
          timer.update(); // Reset delta to avoid a time-jump
          rafRef.current = requestAnimationFrame(loop);
        }
      },
      { threshold: 0 }
    ...
    // Start loop (will self-pause if not visible)
